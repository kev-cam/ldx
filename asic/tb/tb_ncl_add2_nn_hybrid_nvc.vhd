-- tb_ncl_add2_nn_hybrid_nvc.vhd — 2-bit ripple-carry adder (minimal
...
entity tb_ncl_add2_nn_hybrid_nvc is
end entity;

architecture sim of tb_ncl_add2_nn_hybrid_nvc is
  ...
  stim : process
    ...
    begin
      for i in 0 to 1 loop
        abit := (av / (2**i)) mod 2;
        bbit := (bv / (2**i)) mod 2;
        if abit = 1 then aH(i) <= HI; aL(i) <= LO;
        else             aH(i) <= LO; aL(i) <= HI;
        end if;
        if bbit = 1 then bH(i) <= HI; bL(i) <= LO;
        else             bH(i) <= LO; bL(i) <= HI;
        end if;
      end loop;
