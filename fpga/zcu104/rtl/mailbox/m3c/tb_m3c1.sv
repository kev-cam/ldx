...
module tb_m3c1;
  ...
  always @(posedge clk) if (egr_valid && egr_ready) begin
    ...
    else begin $display("M3C1_DISP %0d", egr_data); dispn++; end
  end
  reg [31:0] prog[0:1023]; integer i,nw;
  initial begin
    for(i=0;i<1024;i++) prog[i]=32'hx;
    $readmemh("mb_m3c1.hex", prog);
    ...
    for(i=0;i<120000 && dispn<12;i++) @(posedge clk);
    repeat(50) @(posedge clk);
    $display("captured %0d displays", dispn);
    $finish;
  end
  initial begin repeat(500000) @(posedge clk); $display("TIMEOUT"); $finish; end
endmodule
