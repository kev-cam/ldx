...
module tb_bank;
  ...
  initial begin
    for(i=0;i<4096;i++) prog[i]=0; $readmemh("mb_bank.hex", prog);
    ...
    for(i=0;i<12000000 && !seen;i++) @(posedge clk);
    $display("cycles=%0d for 200 sim-cycles => %0d cyc/sim-cycle  result=%04x", t1-t0, (t1-t0)/200, res);
    $finish;
  end
  initial begin repeat(16000000) @(posedge clk); $display("TIMEOUT"); $finish; end
endmodule
